...
// syncRulesForTenant finds all the PrometheusRules relevant for a MimirTenant and sends them to Mimir
func (r *MimirTenantReconciler) syncRulesForTenant(ctx context.Context, tenant *domain.MimirTenant) error {
	rules, err := r.findPrometheusRulesFromLabels(ctx, tenant.Spec.Rules.Selectors)
	if err != nil {
		return err
	}
	...
	// Synchronize each Rule on Mimir for the tenant
	for ruleName, rule := range unpackedRules {
		if err := sendRuleToMimir(ctx, tenant.Spec.ID, tenant.Spec.URL, ruleName, rule); err != nil {
			return err
		}
	}

	// Find the namespaces on Mimir that are NOT in our list of WANTED rules
	// Those namespaces might have been created earlier by the operator, but the MimirTenant selectors
	// have changed since then, making those namespaces unwanted and in need of deletion.
	namespaces, err := diffRuleNamespaces(ctx, unpackedRules, tenant.Spec.ID, tenant.Spec.URL)
	if err != nil {
		return err
	}
	...
	for _, namespace := range namespaces {
		if err := sendRuleToMimir(ctx, tenant.Spec.ID, tenant.Spec.URL, namespace, ""); err != nil {
			return err
		}
	}
...
func (r *MimirTenantReconciler) deleteRulesForTenant(ctx context.Context, tenant *domain.MimirTenant) error {
	// List all the rules and namespaces for the tenant in JSON format
	json, err := mimirtool.ListRules(ctx, tenant.Spec.ID, tenant.Spec.URL)
	if err != nil {
		return err
	}
	...
	for _, rule := range rules {
		if err := sendRuleToMimir(ctx, tenant.Spec.ID, tenant.Spec.URL, rule.Namespace, ""); err != nil {
			return err
		}
	}
...
func sendRuleToMimir(ctx context.Context, tenantId, url, ruleName, rule string) error {
	...
	err = mimirtool.SynchronizeRule(ctx, ruleName, fileName, tenantId, url)

	// Cleanup after ourselves
	if err := os.RemoveAll(fileName); err != nil {
		log.FromContext(ctx).
			WithValues("mimirTenant", tenantId).
...
func (r *MimirTenantReconciler) findPrometheusRulesFromLabels(ctx context.Context, selector *metav1.LabelSelector) (*prometheus.PrometheusRuleList, error) {
...
func (r *MimirTenantReconciler) unpackRules(list *prometheus.PrometheusRuleList) (map[string]string, error) {
	if list == nil {
